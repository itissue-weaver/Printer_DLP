...
def create_widgets_manual(master, icon_a_up, icon_a_down, icon_rotate, kwargs):
    entries = []
    frame_platform = ttk.LabelFrame(master, text="Platform Control", style="Custom.TLabelframe",  padding=3)
    ...
    ttk.Label(frame_platform, text="Displacement[mm]: ", style="Custom.TLabel").grid(row=1, column=0, sticky="nsew")
    svar_displacement = ttk.StringVar(value="8")
    entries.append(svar_displacement)
    ttk.Entry(frame_platform, textvariable=svar_displacement, style="Custom.TEntry").grid(row=1, column=1, sticky="nsew")
    ttk.Button(
        frame_platform,
        image=icon_a_up,
    ...
    ttk.Label(frame_vat, text="Rotation[°]: ", style="Custom.TLabel").grid(row=0, column=0, sticky="nsew")
    svar_rotation = ttk.StringVar(value="90")
    ttk.Entry(frame_vat, textvariable=svar_rotation, style="Custom.TEntry").grid(row=1, column=1, sticky="nsew")
    ttk.Button(
        frame_vat,
        image=icon_rotate,
    ...
    frame_led = ttk.LabelFrame(master, text="LED Control", style="Custom.TLabelframe", padding=3)
    ...
    ttk.Label(frame_led, text="ON/OFF: ", style="Custom.TLabel").grid(row=0, column=0, sticky="nsew")
    ...
    ttk.Label(frame_led, textvariable=svar_layer, style="Custom.TLabel").grid(row=1, column=1, sticky="nsew")
...
class ManualControl(ttk.Frame):
    ...
    def layer_callback(self):
        ...
        path = filedialog.askopenfilename(initialdir="/", title="Select file", filetypes=(("Image files", ["*.png", "*.jpg", "*.jpeg", "*.gif", "*.bmp"]), ("all files", "*.*")))
        if path:
    ...
    def up_callback(self, displacement):
        if self.thread_motor is None or not self.thread_motor.is_alive():
            ...
            self.thread_motor = threading.Thread(target=control_motor_from_gui, args=("move_z", "cw", "top", "z", int(steps),))
    ...
    def down_callback(self, displacement):
        if self.thread_motor is None  or not self.thread_motor.is_alive():
            steps = 200 * int(displacement) / 8
            self.thread_motor = threading.Thread(target=control_motor_from_gui, args=("move_z", "ccw", "bottom", "z", int(steps),))
            self.thread_motor.start()
        else:
            print("Motor is already running... Wait until it finishes")
            if not self.thread_motor.is_alive():
    ...
    def vat_callback(self, rotation):
        if self.thread_motor is None or not self.thread_motor.is_alive():
            ...
            self.thread_motor = threading.Thread(target=control_motor_from_gui, args=("move_plate", "cw", "top", "plate", int(steps), ))
            self.thread_motor.start()
        else:
            print("Motor is already running... Wait until it finishes")
            if not self.thread_motor.is_alive():
    ...
    def led_callback(self, state=False):
        if self.thread_led is None or not self.thread_led.is_alive():
            ...
            self.thread_led = threading.Thread(target=control_led_from_gui, args=(state,))
